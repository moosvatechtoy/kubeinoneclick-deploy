...
import com.prokarma.oneclick.test.any
...
@SpringBootTest(classes = [com.prokarma.oneclick.config.security.SecurityConfig::class, SecurityConfigIT.FakeRestController::class])
@AutoConfigureMockMvc
@DirtiesContext
@TestPropertySource(properties = ["oneclick.admin-password=admin456"])
class SecurityConfigIT {
    ...
    @MockBean
    lateinit var successHandler: com.prokarma.oneclick.config.security.SuccessHandler

    @MockBean
    lateinit var failureHandler: com.prokarma.oneclick.config.security.FailureHandler

    @MockBean
    lateinit var logoutSuccessHandler: com.prokarma.oneclick.config.security.LogoutSuccessHandler

    @MockBean
    lateinit var accessDeniedHandler: com.prokarma.oneclick.config.security.AccessDeniedHandler

    @MockBean
    lateinit var authenticationEntryPoint: com.prokarma.oneclick.config.security.AuthenticationEntryPoint

    @Test
    fun `security should not authenticate assets access`() {
        mockMvc.perform(get("/assets/img/oneclick.png"))
